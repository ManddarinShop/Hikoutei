...
import type { RowCheckFormulaVocabulary } from "@hikoutei/ikisaki";
import {
...
/**
 * Header of the row-check formula column of a User_Input tab.
 *
 * P8-C sole-source: the header constant and the join semantics live in the
 * contracts leaf (`sheets/rowCheck.ts`); this re-export keeps existing
 * adapter-internal import paths valid. The column sits DIRECTLY AFTER the
 * registered range (outside every range-scoped read, hash, and blank-row
 * rule), carries a per-row token-join formula (contracts
 * `sheets/rowCheck.ts` encoding, `@hikoutei/ikisaki`
 * `evidence/rowCheckFormula.ts` builder) on every
 * system-created row, and lets inbound polling detect human edits by
 * reading only this one narrow column (see `operations/rowChecks.ts`).
 */
export { SYNC_ROW_CHECK_HEADER as GOOGLE_SHEETS_API_ROW_CHECK_HEADER } from "@hikoutei/contracts/sheets/rowCheck.js";

/**
 * Host binding of the neutral row-check formula vocabulary (owned by
 * `@hikoutei/ikisaki` `evidence/rowCheckFormula.ts`): the single-sourced
 * delimiter/separator/tag constants every formula call site passes to the
 * neutral builder, so the formula and the canonical renderer share one
 * source of truth.
 */
export const GOOGLE_SHEETS_API_ROW_CHECK_FORMULA_VOCABULARY: RowCheckFormulaVocabulary = {
  delimiter: SYNC_ROW_CHECK_DELIMITER,
  lengthSeparator: SYNC_ROW_CHECK_LENGTH_SEPARATOR,
  tags: SYNC_ROW_CHECK_TOKEN_TAGS,
};
