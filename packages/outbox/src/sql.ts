 ...
 *
 * The kernel depends on these contracts instead of a particular SQL driver.
 * A host adapter (MikroORM, node:sqlite, or a future provider) satisfies them
 * structurally; the kernel never imports a driver.
 */
...
export interface SqlExecutor {
   ...
   * values into queue contracts; the generic is retained for adapter
...
/**
 * Owns SQL execution contexts for one SQLite-backed queue store.
 *
 * `transaction()` is the only outer write boundary. Every queue mutation that
 * belongs with a business transaction must use the `sql` executor supplied to
 * the callback.
 */
