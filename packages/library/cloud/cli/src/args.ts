...
export const SETUP_HELP_TEXT = [
  ...
  "",
  "Progress is reported to stderr across the ten setup phases (cloud auth,",
  "Drive access, project, APIs, service account, service-account key,",
  "spreadsheet, share, service-account access, output). An overall bar",
  "advances only when a phase actually completes — it is not an ETA and",
  "never guesses a percentage — and a detail line shows the bounded",
  "propagation checks (how many of the eight key/access checks have run)",
  "and the known 2, 4, 8, 16, 30, 30, 30 s waits, with a fixed \"working…\"",
  "label for unknown-duration steps. On an interactive terminal the block",
  "redraws in place; in CI, non-TTY, or NO_COLOR sessions one static line",
  "is printed per phase/retry event with no control sequences. Progress is",
  "paused and the block cleared during the interactive gcloud login",
  "handoff. Progress never prints credentials, tokens, keys, project ids,",
  "emails, paths, or raw command output, and it never changes the setup",
  "result; --dry-run prints the command plan only.",
  "",
