...
export async function runInferCli(input: RunInferCliInput): Promise<number> {
  ...
  try {
    ...
    // Stdout contract is unchanged (block + summary); `--emit` only adds
    // the descriptor-file side effect below.
    input.output.write(`${result.tsBlock}\n\n${result.summary}\n`);
    if (input.options.emitPath !== undefined) {
      try {
        await emitDescriptorFile(
          input.options.emitPath,
          serializeInferDescriptorFile(result),
...
/**
 * Writes the descriptor-file JSON text to `path`.
 *
 * Parent directories are never created: a missing parent fails with
 * `emit_parent_missing`. An existing file is left untouched unless `force`
 * is set (`emit_exists` otherwise). Throws `InferError` with a stable code
 * so the flow maps it to the machine-readable stderr line; the path itself
 * is never echoed (it may embed user directory layout).
 */
export async function emitDescriptorFile(
  path: string,
  json: string,
  options: { readonly force: boolean },
): Promise<void> {
  try {
    await writeFile(path, json, options.force ? {} : { flag: "wx" });
  } catch (error: unknown) {
    const code = (error as { code?: unknown })?.code;
    if (code === "ENOENT") {
      throw new InferError(
        "emit_parent_missing",
        "could not write the descriptor file — the parent directory does not exist.",
      );
    }
    if (code === "EEXIST") {
      throw new InferError(
        "emit_exists",
        "the descriptor file already exists — pass --force to overwrite it.",
      );
    }
    throw new InferError("emit_write_failed", "could not write the descriptor file.");
  }
}
