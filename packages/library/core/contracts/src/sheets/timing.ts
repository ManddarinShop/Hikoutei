/**
 * Kernel-aligned provider-timing contract (canonical tables live in
 * `@hikoutei/ikisaki` `timing/providerTiming.ts`; the kernel re-exports this
 * module so both import paths stay alive).
 *
 * Provider-neutral timing vocabulary for one remote provider operation:
 * lifecycle operation kinds, per-kind counts, and measured phases. Timing
 * is diagnostic only: a sink failure must never change persistence or
 * provider behavior.
 */
