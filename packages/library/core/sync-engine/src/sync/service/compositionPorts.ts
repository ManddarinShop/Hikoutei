 ...
 * concrete-adapter wiring lives in `packages/library/core/composition/src/`, which registers the
 * engine-facing ports below at composition-root load (see
 * `packages/library/core/composition/src/index.ts`; the public API layer is the composition
 ...
 * `packages/library/core/composition/src/index.ts`). Lazy by design: only the thunk is stored at
