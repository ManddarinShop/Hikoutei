...
// Compat re-exports: the shared setup path/fs primitives moved verbatim to
// `setupPaths.ts` — the dependency-free CLI leaf — breaking the previous
// checkpoint ↔ envFileWriter module cycle. The checkpoint module path stays
// stable for existing importers (tests, setupFlow, setupPathCollision).
export {
  SETUP_LOCK_SUFFIX,
  SETUP_STATE_TEMP_SUFFIX,
  defaultSetupStateWriteFs,
...
// Key-material security logic moved verbatim to `keyMaterial.ts`.
// Compat re-exports: tests and keyProvision import these names from the
// checkpoint module path, so the paths stay stable for existing importers.
export {
