#!/usr/bin/env node
/**
 * Express example verification scenario (CI).
 *
 * Spawns `npm start` in the example directory, waits for the server, drives a
 * CRUD scenario over HTTP, then terminates the server. On failure the captured
 * server log tail is printed to stderr and the process exits non-zero.
 *
 * Usage:
 *   node scripts/ci/examples/express.mjs --dir examples/express [--port 3000] [--output result.json]
 */
...
async function main() {
  ...
  try {
    ...
    await step("missing user", async () => {
      const body = await assertJson(await requestJson("GET", `${base}/users/missing`), 404, "GET /users/missing");
      assert(JSON.stringify(body) === JSON.stringify({ error: "not found" }), "GET /users/missing: expected {error:\"not found\"}");
    });

    await step("update user", async () => {
      const body = await assertJson(
        await requestJson("PATCH", `${base}/users/u1`, { name: "Ada Lovelace" }),
        200,
        "PATCH /users/u1",
      );
      assert(body?.name === "Ada Lovelace", "PATCH /users/u1: expected updated name");
    });

    await step("delete user", async () => {
      const { response, text } = await requestJson("DELETE", `${base}/users/u1`);
      assert(response.status === 204, `DELETE /users/u1: expected HTTP 204, got ${response.status}`);
      assert(text === "", "DELETE /users/u1: expected empty body");
    });
