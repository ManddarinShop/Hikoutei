...
import { parseSeed, SeededRandom, deriveSeed } from "./prng.mjs";
import {
  generatePatch,
  generateRow,
  OPERATION_KINDS,
...
/**
 * Drops the library's cached process logger so the pinned env takes
 * effect (see runLocalMultiTableSoak).
 *
 * The internal observability module is deliberately NOT part of the
 * package export boundary (the exports map is pinned to `.` only), so the
 * runner resolves the SAME module instance the library uses per
 * environment instead: under Vitest the absolute source URL loads the
 * `.ts` source module (the library's own test alias), and in the CLI it
 * resolves to the built dist module the package self-reference loads.
 * Both are exact repository-internal files, never a package subpath.
 */
...
/**
 * Validates the COMPLETE resume state schema with stable local reasons.
 *
 * Checks version, run id, seed (integer in the PRNG's [0, 2^32-1] domain,
 * equal to the stored params seed), mode (must equal the CURRENT live/local
 * detection so a resumed run can never silently switch projection
 * semantics), started timestamp, the exact known params (finite numeric
 * fields matching the CLI's positive/non-negative bounds, exact known
 * unique table vocabulary matching tableRows), the cumulative
 * operation/probe/convergence counters with their cross-field consistency
 * constraints, and lastCompletedCycle. Missing, malformed, or UNKNOWN
 * fields fail cleanly; nothing is ignored or defaulted, so a partially
 * valid state can never silently continue as a degraded run.
 *
 * @param {unknown} stored parsed state.json value.
 * @param {"local" | "live"} mode detected current mode.
 * @returns {void} throws with a stable `--resume failed: ...` reason.
 */
function validateResumeState(stored, mode) {
  const fail = (message) => {
 ...
 */
async function readCheckpointOrUndefined(artifacts, state) {
...
function checkpointMarker(runId, cycle, status) {
...
/**
 * Tracks which artifact records already exist so a resumed run can never
 * duplicate JSONL history: cycle records dedupe by cycle, operation
 * records by (cycle, actor, index), resource samples by cycle.
 */
 ...
 */
async function validateResumeHistory(artifacts, state, checkpoint) {
  const fail = (message) => {
    throw new Error(`--resume failed: ${message}`);
  };
 ...
 */
function buildProbeEvidence({ state, activeEntities, cycleByNumber, inFlightCycle, observedByTable }) {
  const replayThrough = inFlightCycle ?? state.lastCompletedCycle;
 ...
 */
function requireProbeEvidenceOrFail(replay) {
 ...
 * @returns {boolean}
 */
function matchAllowedPrefixRow(prefixPlans, id, row, fieldPlan) {
  for (const plan of prefixPlans) {
...
async function rebuildOracleFromSqlite(hikoutei, oracle, activeEntities, tokenByEntity, progress) {
  const em = hikoutei.em.fork();
  for (const entry of activeEntities) {
    const token = tokenByEntity.get(entry.name);
    const rows = await em.find(token, {});
