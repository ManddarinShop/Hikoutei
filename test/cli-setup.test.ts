...
import { confirmSetup, promptLoginHandoff } from "../src/cli/confirm.js";
import {
...
describe("runSetup — failure mapping", () => {
  ...
  it("points the no-account guidance at the exact Drive-enabled re-login command", async () => {
    ...
    // Drive-enabled login, never the bare `gcloud auth login`. The message
    // stays sanitized (no token/key material flows through the auth-list path).
    const dir = makeTempDir();
    const harness = createHarness(dir);
    harness.runnerScript = (args, options) =>
      args[0] === "auth" && args[1] === "list"
    ...
    if (result.status === "error") {
      ...
      // The obsolete bare-login hint must not survive.
      expect(result.message).not.toMatch(/gcloud auth login` first/);
      expect(result.message).not.toContain(SECRET_JWT);
      expect(result.message).not.toContain(SECRET_AUTHORIZATION);
    }
  });
  ...
  it("points the auth-list failure guidance at the exact Drive-enabled re-login command", async () => {
    ...
    // so non-interactive/CI/non-TTY users get the exact Drive-enabled login,
    // never the bare `gcloud auth login`. The message stays sanitized: raw
    // stderr from the failed invocation is never forwarded.
    const dir = makeTempDir();
    const harness = createHarness(dir);
    harness.runnerScript = (args, options) =>
      args[0] === "auth" && args[1] === "list"
        ? failed(1, `could not reach account registry ${SECRET_JWT}`)
        : freshSetupScript(harness.keyPath)(args, options);
    const result = await harness.run();
    expectError(result, SETUP_ERROR_CODES.GCLOUD_NOT_LOGGED_IN);
    if (result.status === "error") {
      expect(result.message).toContain("could not list active gcloud accounts");
      expect(result.message).toContain("gcloud auth login --enable-gdrive-access --force");
      // The obsolete bare-login hint must not survive either branch.
      expect(result.message).not.toMatch(/gcloud auth login` first/);
      // Raw stderr and secrets never reach the message.
      expect(result.message).not.toContain("could not reach account registry");
      expect(result.message).not.toContain(SECRET_JWT);
      expect(result.message).not.toContain(SECRET_AUTHORIZATION);
    }
  });
  ...
  it("keeps install guidance for the auth-list not_found case", async () => {
    ...
    const harness = createHarness(dir);
    harness.runnerScript = (args, options) =>
      args[0] === "auth" && args[1] === "list" ? { status: "not_found" } : freshSetupScript(harness.keyPath)(args, options);
    const result = await harness.run();
    expectError(result, SETUP_ERROR_CODES.GCLOUD_NOT_LOGGED_IN);
    if (result.status === "error") {
      expect(result.message).toContain("cloud.google.com/sdk");
      expect(result.message).not.toMatch(/auth login/);
    }
  });
