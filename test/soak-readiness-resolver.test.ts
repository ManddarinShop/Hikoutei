 ...
 * branches ship `packages/hikoutei-sync-engine/src/sync/service/systemStateReadiness.ts`, but
...
describe("resolveSystemStateReadinessReader (optional capability)", () => {
  ...
  it("falls back to the built dist module for the EXPECTED plain-Node unsupported-TS-loader shape when the source is present", async () => {
    ...
    const sourceUrl = "/repo/packages/hikoutei-sync-engine/src/sync/service/systemStateReadiness.ts";
    const expectedError = Object.assign(
      new Error(
        "Cannot find module '/repo/packages/hikoutei-sync-engine/src/sync/service/systemStateReadiness.js' imported from " +
          sourceUrl,
      ),
