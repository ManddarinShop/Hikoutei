...
상태: **설계 확정 (2026-08-28), Phase 1-3 구현 (PR #385, #386)**

MVP 제약 (terra 리뷰 반영): 어답션 MVP는 관리 컬럼이 **연속 블록 + 엔티티
선언 순서**인 탭만 지원한다. 비연속/재배치/선언 순서 불일치는 dry-run과
adopt 모두 blocked 처리되며, 세그먼트 쓰기 경로와 위치 독립 프로비저닝은
후속 과제다.
...
| D3 | **쓰기 경로 세그먼트화 (MVP 제약 있음)** | 바인딩된 필드가 비연속이면 세그먼트 쓰기가 원칙이나, **MVP 어답션은 관리 컬럼이 연속 블록**이어야 함 (User_Input 폴링/클린업이 registeredRange 위의 위치 기반으로 동작하기 때문). 비연속/순서 불일치/여분 컬럼이 블록 내에 있으면 dry-run과 adopt 모두 blocked 처리 (레포트가 이동을 안내). 세그먼트 지원은 후속 과제. |
| D4 | **PK 정책** | 비즈니스 키(PK)는 폴링 매칭의 기준이므로 시트에 존재해야 한다. 기존 컬럼을 지정(유니크성 검사)하거나, 없으면 **라이브러리가 PK 컬럼을 맨 오른쪽에 append + 기존 행 채워넣기**(기존 셀 이동·수정 없음). **MVP 제약**: PK 헤더는 PK 프로퍼티 이름과 정확히 일치해야 한다 — `identityFrom` 별칭(헤더 ≠ 프로퍼티 이름)은 `IDENTITY_ALIAS_UNSUPPORTED` 블로킹 처리하며, 별칭 지원은 후속 과제. |
