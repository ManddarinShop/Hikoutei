...
/** Maps each public scalar tag to the value stored in the entity. */
type TypedSheetsScalarValueByType = {
  string: string;
  number: number;
  boolean: boolean;
  date: Date;
...
/** Resolves the value represented by one scalar property definition. */
type TypedSheetsScalarPropertyValue<
  Property extends TypedSheetsScalarPropertyDefinition,
> = TypedSheetsScalarValueByType[Property["type"]];

/** Resolves the value represented by an owning relation property. */
type TypedSheetsManyToOnePropertyValue<
  Target extends TypedSheetsEntityClass<object>,
  Nullable extends boolean | undefined,
> = InstanceType<Target> | (Nullable extends true ? null : never);

/** Resolves the value represented by an inverse relation property. */
type TypedSheetsOneToManyPropertyValue<
  Target extends TypedSheetsEntityClass<object>,
...
> = Property extends TypedSheetsScalarPropertyDefinition
  ? TypedSheetsScalarPropertyValue<Property>
  : Property extends TypedSheetsManyToOnePropertyDefinition<infer Target>
    ? TypedSheetsManyToOnePropertyValue<Target, Property["nullable"]>
    : Property extends TypedSheetsOneToManyPropertyDefinition<infer Target>
      ? TypedSheetsOneToManyPropertyValue<Target>
      : never;
