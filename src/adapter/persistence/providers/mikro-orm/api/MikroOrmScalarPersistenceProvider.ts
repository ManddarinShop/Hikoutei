...
export class MikroOrmScalarPersistenceProvider implements ScalarEntityPersistenceProvider {
  ...
  /** Counts through a clean manager without materializing entities. */
  ...
  /** Runs row and count reads in one explicit SQLite snapshot transaction. */
  async readSnapshot<Result>(
    ...
    return this.storage.transactional(async ({ entityManager }) =>
      work(new MikroOrmScalarReader(entityManager, this.bindings, this.bindingsByTable))
    );
  }
...
/** Transaction-bound reader used by reads, counts, and flush-time lookups. */
...
async function readRows(
  ...
  const result: unknown = await Reflect.apply(
    ...
    [
      binding.entity,
      toMikroOrmFilter(binding.descriptor, query.predicate, query.primaryKeyColumn),
...
function toMikroOrmFilter(
  ...
  switch (predicate.kind) {
    case "comparison":
      return {
        [predicate.field]: {
          [`$${predicate.operator}`]: toInternalValue(descriptor, predicate.field, predicate.value),
        },
      };
    case "set":
      return {
        [predicate.field]: {
          [`$${predicate.operator}`]: predicate.values.map((value) =>
    ...
    case "all":
      return {
        $and: predicate.predicates.map((child) =>
          toMikroOrmFilter(descriptor, child, primaryKeyColumn)
        ),
      };
    case "any":
      return {
