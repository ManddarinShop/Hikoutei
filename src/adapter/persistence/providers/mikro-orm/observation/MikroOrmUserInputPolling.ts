...
import { persistPreparedRows } from "./MikroOrmUserInputPollingPersistence.js";
...
export async function pollMappedUserInputWithMikroOrm(
  ...
  const preparedRows = prepareObservedRows(mappings, observed, state, accumulators);
  await persistPreparedRowsIfNeeded(options, state, preparedRows, accumulators);
