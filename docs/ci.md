...
triggered manually by pushing a tag such as `canonical-codec-v0.1.0`. It
validates the tag and package version, runs package tests plus the Node/Apps
Script parity test, checks the public tarball contents, and publishes with
`--access public --provenance`. The `@hikoutei` npm scope and repository
`NPM_TOKEN` must be configured before the first publish.

Codec versioning is intentionally not coupled to the Hikoutei `develop` or
`main` release workflows in this change. Publish the exact codec dependency
before releasing the first Hikoutei package that declares it; the root develop
and stable publish verification jobs fail if the requested codec version is not
available on npm.
