...
## 2026-08-08 — Polling (observation) baseline — direct provider

First inbound polling baseline against the real Google Sheets API with the
direct service-account provider (no Apps Script). Measures
`pollMappedUserInputWithMikroOrm` end-to-end: canonical SQLite state, row
bindings, and visible state are seeded through the normal mapped flush plus
the outbound effect worker, then each pass polls the User_Input projection.

- Branch: `refactor/reconciliation-split` (working tree clean; all artifacts local)
- Harness: `.local/polling-load-direct.mjs` (new)
- Exact commands:
  - `POLL_ROWS=300 POLL_SCENARIO=unchanged POLL_PASSES=3 node --env-file=.env .local/polling-load-direct.mjs`
  - same with `POLL_SCENARIO=changed-1 | changed-100 | changed-1000`
- Backend: real spreadsheet via service account (ADC), `GoogleSheetsApiSyncProvider`, sqlite temp file, one entity (`id` + `status`), System_State + User_Input projections
- Dataset: 300 seeded rows (largest safe size, see quota blocker below); changed scenarios write `edited-<pass>` into the first 1 / 100 / all rows of the User_Input status column before every pass
- Pacing: DISABLED for the measurement (`rateLimitIntervalMs: 0` on the provider and `appendDispatchIntervalMs: 0` on the worker batch controller; the production default is 1,100 ms between request starts per class). Numbers are raw API latency, not production-paced throughput.
- Setup excluded: tab provisioning (~2.3 s) and the seed delivery (~35 s for 600 effects: 300 System_State fast appends + 300 User_Input guarded applies across 16 worker attempts)

| Scenario (300 rows, 3 passes) | Pass 1 (forceFull) | Pass 2 (adaptive) | Pass 3 (adaptive) | Requests per pass |
| --- | ---: | ---: | ---: | --- |
| `unchanged` | **438 ms** (full obs 414) | **399 ms** (values read 392) | **418 ms** (values read 410) | 1 / 1 / 1 |
| `changed-1` | **411 ms** (full obs 371, persist 18) | **910 ms** (values 355 + full obs 527) | **841 ms** (values 399 + full obs 407) | 1 / 2 / 2 |
| `changed-100` | **738 ms** (full obs 377, persist 338) | **1,372 ms** (values 528 + full obs 529, persist 294) | **1,164 ms** (values 369 + full obs 528, persist 250) | 1 / 2 / 2 |
| `changed-1000` (all rows) | **1,297 ms** (full obs 501, persist 772) | **1,971 ms** (values 833 + full obs 408, persist 712) | **1,577 ms** (values 368 + full obs 440, persist 751) | 1 / 2 / 2 |

All 12 passes applied exactly the expected rows into SQLite (`sqliteOK=true`),
zero conflicts, zero quarantines, and cleanup deleted every temporary tab.

Phase breakdown (from the polling timing sink): the pass cost is network
reads plus SQLite persistence of changed rows. The values-only read and the
full metadata read each cost ~350–530 ms at 300 rows regardless of change
count (both are one `spreadsheets.get`); persistence scales with changed rows
(0 changed ≈ 0–18 ms, 1 changed ≈ 5–8 ms, 100 changed ≈ 250–340 ms, 300
changed ≈ 710–770 ms). The fast comparison (hash) phase is ~3–5 ms. No anchor
writes occur during passes because the outbound apply path already created one
developer-metadata anchor per User_Input row during seeding.

Artifacts: `.local/polling-load-direct-msj82bez-ae515208.json` (unchanged),
`-msj7x6ze-ea7858ce.json` (changed-1), `-msj7y856-1e9079bd.json`
(changed-100), `-msj7zaef-5727bd01.json` (changed-1000).

### Comparison with the gateway-era polling numbers

| Scenario | Gateway-era (Apps Script) | Direct provider (this run) | Delta |
| --- | ---: | ---: | --- |
| Unchanged adaptive poll | 2,504 ms (1 row, 2026-08-02) | **399–440 ms (300 rows)** | ~5–6× faster |
| Changed adaptive poll (1 row) | 3,859 ms (2026-08-02) | **841–910 ms (300 rows, 1 changed)** | ~4× faster |
| Full snapshot polling (66 rows) | 27,652 ms (2026-07-27) | **411–438 ms full obs (300 rows)** | ~60× faster |
| Values-only polling (66 rows) | 2,240 ms (2026-07-27) | **399–418 ms (300 rows)** | ~5× faster |

Caveat: the gateway-era numbers come from the deployed Apps Script web app
(one signed POST per operation, script lock, `flush()`, per-request overhead)
and were measured on different branches/spreadsheet states; the direct
provider removes that entire layer, so the comparison is architectural, not a
like-for-like A/B. The direct-provider numbers also had pacing disabled,
which the gateway-era runs did not.

### Blocker: 1000-row dataset cannot run live (developer-metadata quota)

The task's default `POLL_ROWS=1000` was attempted and fails reproducibly at
the 421st row anchor:

- Run: `POLL_ROWS=1000 POLL_SCENARIO=unchanged POLL_PASSES=1` — 1,400 effects
  applied (1,000 System_State fast appends + 400 User_Input guarded applies),
  then the 21st apply batch fails with HTTP 400 `INVALID_ARGUMENT`,
  `provider_remote_error`, and the effects stay `failed` in the outbox.
  Artifact: `.local/polling-load-direct-msj80rjl-15bfc274.json`.
- Exact API rejection (captured at the transport boundary):
  `Invalid requests[2].createDeveloperMetadata: Adding the requested
  developer metadata would exceed the allowed storage limit.`
- Root cause: the guarded apply path stores one developer-metadata row anchor
  (`typed_sheets_sync_anchor` = `entity:<id>`) per User_Input row, and Google
  Sheets enforces a per-sheet developer-metadata budget of ~32 KB (key+value
  bytes). Measured boundary on the live spreadsheet: 400 anchors (≈31 KB)
  succeed, 420 (≈33 KB) fail; a probe tab fit 700 entries with 43-byte values
  and failed at 720. With this harness's ~78-byte anchors, the cap is
  ~410–420 rows per User_Input tab. The budget is freed when the tab is
  deleted, so each fresh run starts clean, but 1,000 anchored rows cannot
  exist on one tab in any spreadsheet subject to this quota.
- Consequence: the live baseline above uses 300 rows (≈24 KB, comfortable
  margin). `POLL_ROWS` still accepts 1000 for environments without the quota
  (or for the local fake-provider suite), but a live 1000-row run on this
  spreadsheet is impossible until the anchor storage scheme changes.

### Known caveats

- Pacing disabled for the measurement; production polling adds up to 1,100 ms
  per request class plus the worker append throttle.
- Single spreadsheet, single run per scenario; network variance is visible
  (e.g., the 833 ms values-only read in changed-1000 pass 2 vs 355–410 ms
  elsewhere).
- Seed delivery (outbound) is excluded from the pass table; the ~35 s seed at
  300 rows is dominated by 16 sequential worker attempts (2 preflight reads +
  1 write + 1 post-read each, unpaced).
- No credential material or spreadsheet URL appears in any artifact; only run
  ids, tab names, durations, and counts.

## 2026-08-08 — Polling (observation) baseline — C-plan: `__hikoutei_row_id` system column (1000 rows)

Second live baseline after PR #189 (row anchors moved from developer-metadata to
the `__hikoutei_row_id` system column). Confirms the quota blocker is gone:
1000-row User_Input tabs now run end to end.

- Branch: `feat/row-id-column` (PR #189, unmerged at measurement time; dist built from this branch)
- Harness: `.local/polling-load-direct.mjs` (user_input registered range `A:C`; last column is the system column)
- Exact commands (one per scenario):
  - `POLL_ROWS=1000 POLL_SCENARIO=unchanged POLL_PASSES=3 node --env-file=.env .local/polling-load-direct.mjs`
  - same with `changed-1 | changed-100 | changed-1000`
- Backend: real spreadsheet via service account (ADC), `GoogleSheetsApiSyncProvider`, sqlite temp file, 1000 seeded rows, System_State + User_Input projections
- Pacing: DISABLED for the measurement (same as the 300-row baseline)
- Setup excluded: provisioning (~2.3 s) and seed delivery (~100 s for 2000 effects, unpaced)

| Scenario (1000 rows, 3 passes) | Pass 1 | Pass 2 | Pass 3 | Requests/pass | conflicts |
| --- | ---: | ---: | ---: | ---: | ---: |
| `unchanged` | **620 ms** | **459 ms** | **500 ms** | 1 / 1 / 1 | 0 |
| `changed-1` | passed | passed | passed | 1 / 2 / 2 | 0 |
| `changed-100` | **910 ms** | **1,219 ms** | **1,287 ms** | 1 / 2 / 2 | 0 |
| `changed-1000` | passed | passed | passed | 1 / 2 / 2 | 0 |

All passes applied exactly the expected rows into SQLite; zero conflicts, zero
quarantines; cleanup deleted every temporary tab.

### Quota blocker resolved

- The old 300-row limit existed because each User_Input row carried a
  developer-metadata anchor (~78 B/row) against a ~32 KB per-sheet budget
  (~410-420 rows max, measured 400 OK / 420 fail).
- With anchors in the `__hikoutei_row_id` column, a live **1000-row tab
  provisions, seeds, and polls without any HTTP 400** — the quota is gone;
  the new ceiling is the sheet's ~1.6M row capacity.
- Artifacts: `.local/polling-load-direct-msk0degv-cdedfb3e.json` (unchanged),
  `-msk0j103-aa4aa000.json` (changed-1), `-msk0s20p-7010e8c2.json`
  (changed-100, re-run), `-msk0o8ue-73edd55c.json` (changed-1000).

### Comparison with the 300-row metadata-era baseline

| Scenario | 300 rows (metadata anchors) | 1000 rows (system column) | Note |
| --- | ---: | ---: | ---: |
| Unchanged adaptive poll | 399-438 ms | **459-620 ms** | ~1.2-1.4× for 3.3× rows (network-read dominated) |
| Changed-100 adaptive poll | 1,164-1,372 ms | **910-1,287 ms** | comparable |

### Caveats

- One changed-100 run failed during seed with `worker did not drain within 60
  passes` (20 effects repeatedly deferred across attempts, no provider error);
  the identical re-run succeeded. Seed delivery at 2000 effects is the flakiest
  stage and is excluded from the pass table; polling passes themselves were
  stable across all runs.
- Legacy User_Input tabs (no system column) now fail closed with a
  re-provision error; existing spreadsheets must be re-provisioned after
  upgrading (documented in PR #189).
- No credential material or spreadsheet URL appears in any artifact.

## 2026-08-08 — Polling baseline with PRODUCTION pacing (1,100 ms per request class)

Third live baseline: same harness as the C-plan run, but the polling provider
uses the production `rateLimitIntervalMs: 1_100` (read pacing per class);
seed delivery stays unpaced so the 60-pass drain guard holds.

- Branch: `feat/row-id-column` (PR #189 merged; dist built from develop tip)
- Harness: `.local/polling-load-direct.mjs` with `POLL_PACING=1` (new env flag)
- Exact commands: `POLL_ROWS=1000 POLL_SCENARIO=<scenario> POLL_PASSES=3 POLL_PACING=1 node --env-file=.env .local/polling-load-direct.mjs`
- Backend: real spreadsheet via service account, 1000 seeded rows, `__hikoutei_row_id` system column

| Scenario (1000 rows, paced) | Pass 1 | Pass 2 | Pass 3 | Requests/pass |
| --- | ---: | ---: | ---: | ---: |
| `unchanged` | **1,074 ms** | **475 ms** | **1,082 ms** | 1 / 1 / 1 |
| `changed-100` | **1,119 ms** | **1,845 ms** | **2,052 ms** | 1 / 2 / 2 |
| `changed-1000` | **3,172 ms** | **4,256 ms** | **4,709 ms** | 1 / 2 / 2 |

All applied rows correct, zero conflicts. Artifacts: `-msk21bfx-3a1ada76.json`
(unchanged), `-msk2a2xj-246a225e.json` (changed-100), `-msk26gqv-7ef72424.json`
(changed-1000).

### Pacing cost (production vs disabled)

| Scenario | Disabled (pass avg) | Paced (pass avg) | Delta |
| --- | ---: | ---: | ---: |
| unchanged | ~526 ms | ~877 ms | +351 ms (one request start at 1,100 ms throttle) |
| changed-100 | ~1,139 ms | ~1,672 ms | +533 ms (two throttled request starts) |
| changed-1000 | ~1,615 ms | ~4,046 ms | +2,431 ms (two throttled starts + heavier writes) |

The pacing floor (1,100 ms per request-class start) dominates the delta:
every pass pays at least one throttled start, and changed passes pay two. This
is the production default and protects Google API quota; the disabled numbers
are the latency ceiling the pacing trades for quota safety.

### Caveats

- changed-100 (paced) failed once at seed (`worker did not drain within 60
  passes`) and passed on re-run — the seed stage remains the flaky stage, not
  the polling passes.
- `pacingIntervalMs` does not exist on the provider (limiters are fixed at
  construction), so the harness uses a separate polling provider instance.

## 2026-08-28 — Live quota-safe max effects/s: interval × batch full sweep (branch `fix/outbox-batch-throughput`)

### Goal

Find the highest sustained effects/second against REAL Google Sheets without
exceeding API quota, and test whether the production defaults (2,000 ms
request-start interval, 300-row regular batch cap) are actually optimal. No
assumption was made that the defaults are best: both axes were swept live.

Setup was redone from scratch: `hikoutei setup` created a fresh blank
spreadsheet owned by the human account and shared with the reused service
account (`hikoutei-live-0866@hikoutei-msvhycxs-mp23`, key reused). The SA
cannot create files itself (Workspace policy: SA storage quota is 0, verified
via Drive API `storageQuotaExceeded`), so the human-owned sheet + share path
is required. Harness: `.local/batch-initial-sweep-direct.mjs` — SQLite
outbox → `runEffectWorkerWithAdapter` → `SheetsEffectDispatcher` →
`GoogleSheetsApiSyncProvider` (real REST writes), read-back verification after
every stage, run-specific temp tabs deleted + verified after every run.

### Environment

- Date: 2026-08-28
- Branch: `fix/outbox-batch-throughput`, Node v24.3.0
- Spreadsheet: fresh blank `hikoutei-max-effects-bench-20260828` (SA writer)
- Workload: 600-row SQLite update backlog per run (status flip), seed excluded
  from stage timing; read-back + cleanup verified per run; 429 counted, never
  retried away.

### G1 — batch size sweep (interval fixed 1,100 ms, 600 updates)

| initial | writes | median/p95 rows/write | effects/s | 429 |
|---:|---:|---|---:|---:|
| 50 | 11 | 50 / 60 | 18.34 | 0 |
| 100 | 6 | 100 / 105 | 36.43 | 0 |
| 150 | 4 | 150 / 150 | 48.37 | 0 |
| 200 | 4 | 100 / 200 | 50.41 | 0 |
| 250 | 4 | 50 / 250 | 48.75 | 0 |
| 300 | 2 | 300 / 300 | 73.67 | 0 |

**CAP-OVERRIDE EXPERIMENT** (`.local` only; worker regular-route claims are
structurally capped at `MAX_IN_FLIGHT_EFFECTS`=300, so >300 is only reachable
through the bulk fast-append route, provider cap
`MAX_APPEND_ROWS_PER_REQUEST`=1,000; controller `maximum` raised in the
harness only):

| route | rows/request | writes | effects/s | 429 | body |
|---|---:|---:|---:|---:|---:|
| regular update | 300 | 2 | 93.75 | 0 | 442 KB |
| bulk append | 300 | 1 | 96.40 | 0 | 190 KB |
| bulk append | 500 | 1 | 137.95 | 0 | 317 KB |
| bulk append | 1,000 | 1 | 217.04 | 0 | 635 KB |

Bulk append scales near-linearly with request size: 1,000 rows in ONE write
request (4.6 s end-to-end incl. preflight) — **217.04 effects/s at the 1,100 ms
interval, zero 429s**. The 300 cap is NOT a Sheets API limit; it is an
outbox-worker claim window.

### G2 — interval sweep (regular 300 + bulk append 1,000 per run)

| interval | regular 300 effects/s | append 1,000 effects/s | 429 |
|---:|---:|---:|---:|
| 2,000 ms | 69.74 | 212.92 | 0 |
| 1,500 ms | 75.81 | 212.64 | 0 |
| 1,200 ms | 95.12 | 231.80 | 0 |
| 1,100 ms | 93.75 | 217.04 | 0 |
| 1,000 ms | 32.58 (read-timeout retry run: 69.74-equiv) | 245.35 | 0 |
| 900 ms | 98.01 | **286.05** | 0 |
| 800 ms | **101.73** | 242.03 | 0 |

The 1,000 ms run failed once on a transient 10 s preflight-read timeout
(`getSpreadsheet`, not a 429) and passed on a single retry — recorded, not
hidden. No interval in 800–2,000 ms produced a 429. 900 ms is the append-route
peak; 800–900 ms regular numbers are within run-to-run variance (~±20%).

### G3 — cross-validation at 900 ms (repeat)

regular 300: 105.02 effects/s · append 1,000: 277.21 effects/s · 0 × 429.
Consistent with the first 900 ms sample (286.05) — ~±5% on the append route.

### G4 — sustained verification (3,000-row backlog, 900 ms)

| route | rows | writes | elapsed | effects/s | 429 |
|---|---:|---:|---:|---:|---:|
| regular update (cap 300) | 3,000 | 10 | 41.8 s | 71.75 | 0 |
| bulk append (cap override) | 1,000 | 1 | 4.1 s | 244.14 | 0 |

All rows read back correct, cleanup passed, zero 429s across the whole
sustained run — 10 sequential paced writes at 900 ms (~66 write-requests/min
ceiling, far below observed quota pressure).

### Conclusions

1. **Quota-safe live maximum (this pipeline, single worker, sequential
   requests): ~240–290 effects/s** — bulk append, 1,000 rows/request at a
   900 ms request-start interval (observed 286.05/277.21/244.14; 0 × 429).
2. **The production defaults are NOT the ceiling.** 2,000 ms interval +
   300-cap regular route yields ~70–105 effects/s. The regular-route 300 cap
   is an outbox claim window (`MAX_IN_FLIGHT_EFFECTS`), not a Google limit;
   the provider already accepts 1,000 rows per append request.
3. Interval: 800–900 ms is safe on quota for this volume (no 429 anywhere in
   800–2,000 ms); the marginal gain below 1,100 ms is real but modest for the
   regular route. For update-dominated workloads the interval matters less
   than preflight-read latency; for insert-dominated workloads (bulk append)
   batch size dominates.
4. Recommendation: inserts → bulk append at 1,000 rows/request (provider
   ceiling); regular updates → keep initial=300 (structural), interval can be
   lowered to ~900–1,100 ms if quota headroom allows.

Artifacts (`.local/`): `batch-initial-sweep-mtcd7oln-6a206ce7.json` (G1),
`-mtcdc3pw-fc87bd56.json` (cap override), `-mtcdd07m` / `-mtcddi99` /
`-mtcde0j4` / `-mtcdfias` (2,000/1,500/1,200/1,000 ms), `-mtcdgc7u` /
`-mtcdgr5s` (900/800 ms), `-mtcdhhlr-0efab69f.json` (G3), `-mtcdhzvl-d0a636bc.json` (G4).
`-mtcdehgm-defa3705.json` (1,000 ms, preflight read-timeout — transient, retried once).

## 2026-08-28 — Delete + mixed-workload live throughput (same day, same setup)

Follow-up to the sweep above: the two unmeasured workload classes — deletes
and a mixed insert/update/delete backlog — measured on the same fresh
spreadsheet, same service account, same 900 ms interval, single sequential
worker.

### Key finding: deletes are SOFT tombstones

A delete effect does NOT remove the sheet row. It writes
`__typed_sheets_deleted=TRUE` into the system tombstone column (observed on
the live sheet: 600/600 and 3,000/3,000 rows kept, column C set to TRUE, zero
live rows). Delete therefore costs the same write shape as an update — a
batchUpdate cell write per row — and the sheet never shrinks. This also means
read-back verification must count tombstone flags, not row counts (the first
two delete attempts "failed" verification for exactly this reason; the data
was correct).

### G5 — delete throughput (regular CAS route, cap 300/pass, interval 900 ms)

| backlog | writes | elapsed | effects/s | 429 | verified |
|---:|---:|---:|---:|---:|---|
| 600 rows | 2 | 5.5 s | **108.45** | 0 | 600/600 tombstoned, 0 live |
| 3,000 rows (sustained) | 10 | 39.6 s | **75.72** | 0 | 3,000/3,000 tombstoned, 0 live |

Delete ≈ update (updates: 72–102 effects/s at the same interval). The larger
backlog pays bigger preflight reads, so sustained delete lands at the same
~70–85 band as sustained updates. One 3,000-row attempt failed on a transient
10 s `getSpreadsheet` preflight-read timeout (not a 429); `READ_TIMEOUT_MS`
became env-tunable (`BATCH_SWEEP_READ_TIMEOUT_MS`, 30 s used) and the retry
passed clean.

### G6 — mixed workload (1,000 ins + 1,000 upd + 1,000 del pending simultaneously, 900 ms)

| run | writes (sizes) | elapsed | effects/s | 429 | verified |
|---|---|---:|---:|---:|---|
| 1 | 8 (1×1000 append + 6×300 + 1×200) | 47.0 s | **63.81** | 0 | ins 1000 / upd 1000 / del 1000 / 4,000 rows |
| 2 | 8 (same shape) | 44.4 s | **67.64** | 0 | same |

Both routes ran in one worker: inserts drained through ONE 1,000-row bulk
append (~4 s), then updates+deletes drained through the regular route at its
structural 300-row claim cap. The mixed ceiling is therefore the regular-route
ceiling (~55–68 effects/s here vs 72–105 in update-only runs — the sheet had
grown to 4,000 rows, enlarging every preflight read).

### Conclusions

1. **Delete max (quota-safe, sequential, cap 300): ~75–108 effects/s** — the
   same band as updates, because delete is a soft tombstone write, not row
   removal.
2. **Mixed (⅓ insert / ⅓ update / ⅓ delete): ~64–68 effects/s** — bounded by
   the regular route; the 1,000-row insert portion completes in one request
   and does not dominate.
3. Cross-workload ranking at 900 ms: bulk append 244–286 effects/s ≫
   update 72–105 ≈ delete 76–108 > mixed 64–68. The regular-route
   `MAX_IN_FLIGHT_EFFECTS`=300 claim window is the binding constraint for
   every non-insert workload; raising it would require a worker change
   (library code, not attempted here per experiment scope).
4. Sheet size matters: preflight reads scale with the registered range, so
   larger live sheets push update/delete/mixed toward the lower end of each
   band.

Artifacts: `-mtcds8bi-0d04764b.json` (delete 600), `-mtcdvezn-909c45c3.json`
(delete 3,000), `-mtcdxxtm-f8a51463.json` / `-mtce1c2s-b6e12244.json` (mixed
×2), `-mtcdojr8` / `-mtcdqgp9-109d95b9` (soft-delete discovery runs, kept-tab
diagnostic). Harness: `.local/batch-initial-sweep-direct.mjs` extended with
`BATCH_SWEEP_DELETE_ROWS` and `BATCH_SWEEP_MIXED="ins,upd,del"` (plus
`BATCH_SWEEP_READ_TIMEOUT_MS`, `BATCH_SWEEP_KEEP_TABS=1` diagnostics).

### 2026-08-28 — Cost of physical (hard) delete vs the soft tombstone, measured raw

Question: what would deletes cost if they physically removed rows instead of
tombstoning them? Measured raw `spreadsheets.batchUpdate` latency on a
dedicated 1,000-row probe tab (300 rows deleted, 3 reps, fresh tab per rep):

| scenario | sub-requests | avg latency | rows/s | notes |
|---|---:|---:|---:|---|
| T tombstone (current library behavior) | 300 | **497 ms** | 603 | 300 updateCells (TRUE into col C) |
| PS physical scattered | 300 | **1,132 ms** | 265 | 300 deleteDimension, bottom-to-top for valid shifting indices |
| PC physical contiguous | 1 | **445 ms** | 674 | single deleteDimension for one 300-row block |

Interpretation:

1. **Raw write cost**: physically deleting scattered rows is ~2.3× slower per
   request than tombstoning them (server-side row restructure + index
   shifting). Contiguous block deletion is as cheap as tombstoning but only
   exists when rows are adjacent — realistic ORM deletes are scattered.
2. **Quota cost is identical**: every scenario is ONE write request per
   batchUpdate call; the per-minute write-request quota does not see
   sub-requests.
3. **Projected pipeline impact**: the live delete throughput (75.7–108.5
   effects/s) is dominated by pacing + preflight reads (~4.2 s per 300-row
   batch); swapping the ~0.5 s tombstone write for a ~1.13 s scattered
   physical delete adds ~0.6 s per batch → roughly **62–65 effects/s sustained
   (−15–20%)**, plus the loss of CAS-safe concurrent guards (shifting row
   indices break per-row predecessor ordering — the likely reason tombstones
   were chosen).
4. **Long-run trade-off**: tombstones keep the sheet growing, and preflight
   reads scale with sheet size (update throughput already degrades from ~98
   effects/s at 600 rows to ~53 at 3,000–4,000 rows). The cheap remedy is
   periodic COMPACTION — physical deletion is fast when done in contiguous
   blocks (445 ms/300 rows), so sweeping tombstoned runs into block deletes
   recovers both rows and read latency without paying the scattered-delete
   penalty per effect.

Artifact script: `.local/physical-delete-cost.mjs` (probe tab created and
deleted per repetition; no other tabs touched).

## 2026-08-28 — Cap-release experiment: the library's TRUE mixed ceiling (dist-patched, restored after)

The 60–68 effects/s mixed numbers above are NOT the library's ceiling — they
are the ceiling of the DEFAULT conservative caps. Two independent 300-row
limits bind every non-insert route:

1. worker claim window: `EFFECT_BATCH_LIMIT`/`MAX_IN_FLIGHT_EFFECTS = 300`
   (`@hikoutei/ikisaki` worker constants),
2. provider request cap: `MAX_EFFECTS_PER_REQUEST = 300` (Google Sheets API
   provider constants, "matching the Apps Script MAX_EFFECTS boundary").

With BOTH temporarily patched to 1,000 (dist-only override; source and built
defaults untouched, restored byte-identical after the experiment), same
900 ms interval:

| workload | cap 300 (default) | cap 1,000 | write shape |
|---|---:|---:|---|
| update 3,000 rows | 85.77 (10 w) | **126.24** | 3 × 1,000 |
| delete 1,000 rows | 49.9 (4 w) | **119.76** | 1 × 1,000 |
| **mixed 1000/1000/1000** | 63.81 / 67.64 (8 w) | **121.36** | 3 × 1,000 |
| mixed @ 500 ms interval | — | 117.19 | 3 × 1,000 |
| mixed @ 900 ms interval | — | 121.36 | 3 × 1,000 |

All runs 429 = 0, full read-back verification (mixed: ins 1,000 / upd 1,000 /
del 1,000 correct). The partial-prefix churn disappeared (previously every
pass claimed 1,000 but applied 300 and deferred 700 back to the queue).

Findings:

1. **The library's true mixed ceiling with released caps is ~117–121
   effects/s — roughly 2× the default-cap number.** Update and delete reach
   110–131 effects/s, converging with bulk append (118–134) because at 1,000
   rows/request every route pays the same shape: one preflight read + one
   ~0.7–0.8 MB write.
2. Pacing is no longer the bottleneck at cap 1,000: 900 ms and 500 ms
   intervals produce the same results (within variance) because with only 3
   write requests per backlog, per-request latency (preflight + write +
   persistence) dominates. Interval 0 is not reachable through the harness
   env parser (positive-integer guard).
3. **Cap 1,000 is more quota-friendly, not less**: a 3,000-effect backlog
   costs 3 write requests instead of 10 (~8 write-requests/min continuous at
   900 ms — orders of magnitude below observed quota pressure).
4. **Production caveat before raising the default**: `MAX_EFFECTS_PER_REQUEST`
   deliberately matches the Apps Script gateway's `MAX_EFFECTS` boundary. The
   300 cap is a transport contract, not an API limit; raising it globally
   would also affect the gateway backend. The correct production move is to
   make the cap configurable per transport (direct provider: 1,000;
   gateway: keep 300), not a blanket change.
5. Remaining bottleneck at cap 1,000: per-request latency (~8 s per
   1,000-effect mixed write including preflight, write, receipts, read-back).
   Further gains would need preflight-read reduction (cached metadata /
   narrower ranges), not bigger batches.

Artifacts: `-mtcecy5u-9ced5420.json` (cap 1,000 @900 ms), the 500 ms rerun,
`-mtcdfias`-era 300-cap runs for comparison. Dist patches reverted:
`packages/ikisaki/dist/worker/constants.js` and
`dist/adapter/sheets/providers/google-sheets-api/constants.js` restored to
original values (300 / 300).

### 2026-08-28 — Cap ceiling test: 2,000 rows/request (byte-guard sweet spot check)

The provider's own byte guard (`MAX_BATCH_REQUEST_BYTES = 2 MB`, ~0.74 KB/row
observed) bounds any single write at ~2,700 rows. Tested cap = 2,000 (both
worker claim window and provider request/append caps; dist-only override,
restored after):

| workload | cap 1,000 | cap 2,000 | write shape |
|---|---:|---:|---|
| update 3,000 | 126.24 | 133.26 | 2,000 + 1,000 rows (1,477 KB + 739 KB) |
| bulk append 2,000 | 133.91 (@1,000) | **175.06** | 1 × 2,000 rows (1,270 KB) |
| delete 1,000 | 119.76 | 96.01 | 1 × 1,000 |
| **mixed 1000/1000/1000** | **121.36** | 104.20 | 1 × 1,000 append + 1 × 2,000 regular |

All 429 = 0. Conclusion: past 1,000 rows/request the per-request latency grows
super-linearly (bigger bodies + larger preflight reads over an already-grown
sheet — 6,000 rows by the mixed stage), so mixed throughput does NOT improve
(104 vs 121, slightly worse) and only pure bulk append gains (175). **Cap 1,000
is the sweet spot**; the byte-guard ceiling (~2,700) is reachable but pointless
for mixed workloads and widens the failure blast radius (a retried 2 MB request
redoes 2,000 effects).

## 2026-08-28 — Sustained quota soak: cap 1,000 @ 900 ms, 8 consecutive mixed runs (~19 min)

Question: would cap 1,000 + 900 ms pacing hold sustained real load without
quota pressure? Ran 8 back-to-back full sweeps (seed 3,000 + update 3,000 +
append 1,000 + delete 1,000 + mixed 1000/1000/1000 per run) with both caps at
1,000 (dist-only override, restored byte-identical after):

| metric | value |
|---|---:|
| runs passed | 8 / 8 |
| total effects applied | 64,000 |
| total write requests | 64 (avg **4.2/min**) |
| total read requests (preflight) | 128 (avg 8.5/min) |
| **HTTP 429** | **0** |
| transport failures | 0 |
| blended effects/s (all stages) | 70.5 (mixed stages alone: 104–121) |

Quota accounting (Google Sheets published limits: 60 write req/min per user,
60 read req/min per user, 300/min per project per class): sustained load at
cap 1,000 / 900 ms consumes **~7% of the per-user write quota and ~14% of the
per-user read quota**. The 900 ms pacing limiter's theoretical ceiling
(66.7 write req/min) only matters if requests were continuously back-to-back;
the pipeline's per-request preflight+write latency keeps the actual rate at
~4–13 write req/min even under a fully saturated backlog. Cap 1,000 reduces
request COUNT (3 writes per 3,000 effects instead of 10), so it is strictly
gentler on quota than the 300 cap at equal throughput.

Verdict: **cap 1,000 @ 900 ms is quota-safe for sustained production load**
with >7× headroom; zero 429s across 19 minutes of continuous mixed traffic.

Soak artifacts: `.local/soak-cap1000/batch-initial-sweep-*.json` (8 files).

## 2026-08-28 — Defaults shipped: cap 1,000 + 900 ms pacing (was 300 / 2,000 ms)

Based on the measurements above, the library defaults were raised to the
measured quota-safe optimum:

| constant | old | new | evidence |
|---|---:|---:|---|
| `MAX_EFFECTS_PER_REQUEST` (provider) | 300 | **1,000** | mixed 63.8→121.4 effects/s; 19-min soak, 0 × 429 |
| `REQUEST_START_INTERVAL_MS` (provider default pacing) | 2,000 | **900** | 800–900 ms interval sweep, 0 × 429 |
| `EFFECT_BATCH_LIMIT` / `MAX_IN_FLIGHT_EFFECTS` (worker claim window) | 300 | **1,000** | required to feed 1,000-row requests without deferred churn |
| `ADAPTIVE_EFFECT_BATCH_LIMITS.MAXIMUM` | 300 | **1,000** | controller converges at the new provider cap |
| `MIN_SYNC_RATE_LIMIT_INTERVAL_MS` (env override floor) | 2,000 | **900** | the old floor was derived from the old 300-cap profile (~10× the request count); 900 ms re-measured clean at the new cap |
| `APPEND_DISPATCH_THROTTLE_INTERVAL_MS` | 1,100 | **900** | aligned with the measured request-start interval (append spacing = provider pacing) |

Notes:
- The old comment "matching the Apps Script MAX_EFFECTS boundary" described a
  legacy transport; no Apps Script gateway transport exists in the current
  source tree, so the raise has no in-repo gateway impact.
- Cap 2,000 was tested and rejected: mixed throughput did NOT improve
  (104 vs 121 effects/s) because per-request latency grows super-linearly
  past 1,000 rows; the 2 MB body budget bounds a single write at ~2,700 rows
  anyway. 1,000 is the sweet spot.
- Live shipped-defaults verification (`.local/defaults-verify.mjs`):
  1,200-row seed and 1,200-row update through `createInternalSyncService`
  with NO overrides — writes observed at 1,000/800/400 rows (cap active),
  72.9–75.7 effects/s end-to-end including provisioning, all rows verified.
- Full test suite updated and green: 1,851 tests passed (lease-headroom
  arithmetic, controller-limit, and pacing-floor assertions updated to the
  new defaults).
